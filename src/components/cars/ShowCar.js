...
const socket = io.connect("https://carbids.onrender.com");
  ...
  const carComments = allComments?.map((comment, index) => (
    <Container className="fluid">
      <Card key={index}>
        ...
        <Card.Body>{comment.note}</Card.Body>
        <Card.Footer>
          {user.email === comment.email ? (
            <Button
              onClick={() => deleteComment(user, comment._id, car._id)}
              className="m-2"
            >
              Delete
            </Button>
